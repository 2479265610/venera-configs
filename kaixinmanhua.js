/**
 * 开心漫画 (www.kaixinman.com) —— Venera 漫画源 v1.0.0
 * HTML 站，与可乐漫画同 CMS 模板；列表/详情/搜索为 HTML 解析。
 * 阅读页 var params 用与可乐漫画完全相同的 AES-128-CBC 密钥加密：
 *   key = "5V&RoR%Jf@pJPydF", iv = "a79bfa9267e56d57951f5bebf9516ee2"
 * 解密后为 {"chapter_images":[相对路径...], "images_hosts":[域名前缀...]}，图片 = hosts[0] + images[i]。
 *
 * 已实测：
 *   列表/分类/搜索 DOM： div.col-md-6.col-sm-4.col-xs-3 卡片 -> a(/comic/<id>) + .comic-cover.lazy[data-original] + h4
 *   详情页 DOM：       h1 / .comic-cover img[data-original] / a[href*=/category/theme/] / .chapter-list li a[href*=/chapter/]
 *   阅读页：           var params = "..."(加密) -> 解密得 chapter_images + images_hosts
 */
class kaixinmanhua extends ComicSource {
  name = "开心漫画";
  key = "kaixinmanhua";
  version = "1.0.0";
  minAppVersion = "1.0.0";
  ...
  // 列表卡片解析：div.col-md-6.col-sm-4.col-xs-3 -> a(/comic/<id>) + .comic-cover.lazy + h4
  _list(doc) {
    var comics = [], seen = {};
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      var title = "";
      var h1 = doc.querySelector("h1"); if (h1) title = h1.text.trim();
      if (!title) { var sp = doc.querySelector("h1 span"); if (sp) title = sp.text.trim(); }
      ...
      var desc = "";
      var dm = html.match(/class="desc[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
    ...
    onImageLoad: (url) => ({ url: url, headers: { "User-Agent": "Mozilla/5.0 Chrome/126", "Referer": "https://www.kaixinman.com/", "Accept": "image/webp,image/*" } }),
